...
window.addEventListener('load', function async() {
    ...
    function searchScreen() {
        ...
        $('searchMsg').textContent =
            'Finde die Wetterdaten für einen bestimmten Ort:';
    ...
    function search() {
        ...
        if (
            ...
            searchQuery =
                'zip=' + $('searchZip').value + ',' + $('searchCountry').value;
        } else if (
            $('searchCoordLo').value !== '' &&
            $('searchCoordLa').value !== ''
        ) {
        ...
        fetch(searchUrl)
            ...
            .catch(alert('eingegebener Ort ungültig.'));
    }
    ...
    function loginScreen() {
        ...
        $('loginMsg').textContent =
            'Melde dich an und speicher deine Lieblingsstädte!';
    ...
    function registerUser() {
        ...
        if (localStorage.getItem(username) !== null) {
            alert(
                'Der Nutzername ist leider schon vergeben. Bitte wähle einen anderen!'
            );
        } else {
        ...
        if (username === '') {
            ...
            if (localStorage.getItem(username) !== null) {
                ...
                if (currentUser.password === password) {
                    ...
                    // ausgegeben.
                    alert(
                        'Falsches Passwort. Bitte gebe das richtige Passwort ein.'
                    );
                }
            } else {
    ...
    async function callHomeScreen(fav) {
        ...
        $('sunRise').textContent =
            'Sonnenaufgang: ' + convertTime(data.sys.sunrise) + ' Uhr';
        $('sunSet').textContent =
            'Sonnenuntergang: ' + convertTime(data.sys.sunset) + ' Uhr';
        ...
        function checkFavorite(id) {
            ...
            user.favorites.forEach(favorite => {
                if (favorite === id) ifAddNeccessary = false;
            });
        ...
        function getLogoutButton() {
            ...
            logoutIcon.src =
                'https://img.icons8.com/clouds/100/000000/export.png';
        ...
        class DayObj {
            constructor(wkd, dailyTemp, icon, weekDailyCurve, weatherData) {
                ...
                this.getHourlyData = weatherIndex => {
                    return (
                        'Windstärke: ' +
                        weatherData[weatherIndex][0] +
                        ' \n Windrichtung: ' +
                        weatherData[weatherIndex][1] +
                        ' \n Niederschlag: ' +
                        weatherData[weatherIndex][2] +
                        ' \n Luftdruck: ' +
                        weatherData[weatherIndex][3] +
                        '\n Bewölkung: ' +
                        weatherData[weatherIndex][4] +
                        '\n Sichtbarkeit: ' +
                        weatherData[weatherIndex][5] +
                        '\n Sonnenaufgang: ' +
                        weatherData[weatherIndex][6] +
                        ' \n Sonnenuntergang: ' +
                        weatherData[weatherIndex][7]
                    );
                };
            }
        }
